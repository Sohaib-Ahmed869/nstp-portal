...
const Clearance = () => {
    ...
    const [clearanceRequests, setClearanceRequests] = useState([
        ...
        { id: "3", tenantName: "Hexler Tech", dateRequested: "2021-10-10", status: "Pending", applicantName: "John Doe", applicantCnic: "12345-1234567-1", applicantContact: "0333-1234567", officeNumber: "123", vacatingDate: "2021-10-15", reasonForLeaving: "Moving to a new office" },
    ]);
            ...
            <div className={`bg-base-100 mt-5 ring-1 ring-gray-200 p-5 pb-14 rounded-lg ${loading && 'hidden'}`}>
                ...
                <div className="">
                    {sortedRequests.map((request) => (
                        <div key={request.id} className="bg-white rounded-lg shadow-md ring-1 ring-base-200 p-5 mb-5">
                            ...
                            {expandedRequestId === request.id && (
                                <div className="mt-5 pt-5 border-t-primary border-t-2 grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <p className="text-lg font-semibold">Applicant: {request.applicantName}</p>
                                    <p className="text-md">CNIC: {request.applicantCnic}</p>
                                    <p className="text-md">Contact: {request.applicantContact}</p>
                                    <p className="text-md">Office Number: {request.officeNumber}</p>
                                    <p className="text-md">Vacating Date: {request.vacatingDate}</p>
                                    <p className="text-md">Reason for Leaving: {request.reasonForLeaving}</p>
                                </div>
                            )}
                        </div>
                    ))}
