...
import { ICONS_ARRAY, getServiceIcon  } from "../../util/service";
...
const Services = () => {
  ...
  const saveEditedService = async () => {
    ...
    try {
      ...
      const response = await AdminService.editService(
        currentServiceId,
        newService.name,
      ...
      newService.icon = getServiceIcon(newService.icon);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await AdminService.addService(
        tower.id,
        newService.name,
        newService.description,
        newService.icon
      ); // add ", newService.iconNumber" of somin to this
      console.log(response);
      if (response.error) {
        console.log(response.error);
        showToast(false, response.error);
  ...
  const handleEdit = (serviceId) => {
    ...
    setNewService({
      ...
      icon: ICONS_ARRAY.indexOf(serviceToEdit.icon) + 1, // Map the icon component to its index
    });
    setIsEditMode(true);
      ...
      <dialog id="delete_service_modal" className="modal">
        <div className="modal-box">
          ...
          <div className="alert-warning">
            This action may result in unexpected consequences or behaviour.
          </div>
          <div className="modal-action">
            <button
      ...
      >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
          {services.map((service) => (
            <div
              key={service.id}
              className={`relative card p-5 rounded-lg transition-all duration-300 ${
                expandedServiceId === service.id ? "transform scale-95" : ""
              }`}
            >
              <div
                className="absolute top-0 p-3 rounded-tr-md rounded-br-md right-0 h-full bg-primary flex items-center justify-center cursor-pointer"
                onClick={() => toggleExpand(service.id)}
              >
                <ChevronRightIcon
                  className={`size-6 text-base-100 transition-transform duration-300 ${
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      document
                        .getElementById("delete_service_modal")
                        .showModal();
