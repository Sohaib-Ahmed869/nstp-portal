...
const EditEmployeeProfileModal = ({ employeeProfileSelected, setEmployeeTableData }) => {
    ...
    useEffect(() => {
        if (employeeProfileSelected) {
            ...
            setShowContractDuration(employeeProfileSelected.employee_type === 'Contract');
        ...
        <dialog id="edit_employee_profile_modal" className="modal">
            <div className="modal-box w-10/12 max-w-5xl">
                    ...
                    <div className="border-r-2 border-primary">
                        <div className="avatar">
                            <div className="lg:w-44 max-md:w-20 rounded-full">
                                <img src={editedProfile.photo || 'https://via.placeholder.com/150'} alt="Employee Photo" />
                            </div>
                        </div>
                        <button className="btn btn-secondary mt-4" onClick={() => document.getElementById('photo_upload_options').classList.toggle('hidden')}>
                            Change Photo
                        </button>
                        <div id="photo_upload_options" className="hidden mt-4 mr-4">
                            ...
                            <div className={`${fileSelected && "hidden"}`}>
                                <p className="text-sm font-semibold mb-3">Or enter photo URL:</p>
                                <FloatingLabelInput
                                    name="photo"
                                    type="text"
                                    id="photo"
                ...
                <div className="modal-action">
                    <button className={`btn mr-1 ${modalLoading && "btn-disabled"}`} onClick={() => document.getElementById('edit_employee_profile_modal').close()}>Cancel</button>
